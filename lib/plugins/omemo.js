...
export class OmemoClient {
  ...
  async handleDeviceList(msg) {
    ...
    await this.storeDeviceIds(new JID(msg.from).bare, devices);
  }

  async storeDeviceIds(jidBare, devices) {
    console.log("[OmemoClient][storeDeviceIds]", jidBare, devices);

    await this.store.storeDeviceIds(jidBare, devices);
  }

  async start() {
    console.log("[OmemoClient][start]");
  ...
  async getAnnouncedDeviceIds(jid, force = false) {
    ...
    // console.log('[OmemoClient][getAnnouncedDeviceIds]', jid, `force: ${force}`);
    ...
    if (force || !this.subscriptions.has(jid)) {
      try {
        ...
        console.error(`[OmemoClient][getAnnouncedDeviceIds] subscribe to user ${jid}`, e);
      }
      this.subscriptions.add(jid);
    }

    if (!force && await this.store.hasDeviceIds(jid)) {
      console.log('[OmemoClient][getAnnouncedDeviceIds]', jid, ', return from store');
      return await this.store.getDeviceIds(jid);
    }

    let deviceList;
    try {
      deviceList = await this.client.getItems(jid, 'urn:xmpp:omemo:1:devices');
      this.actualizedOpponentDevices.add(jid);
      console.log('[OmemoClient][getAnnouncedDeviceIds] deviceList from server', deviceList);
    } catch (e) {
  ...
  async getDeviceKeyBundle(recipient, registrationId) {
    let keyBundle;
    try {
      keyBundle = await this.client.getItems(typeof(recipient) === 'string' ? recipient : recipient.bare, 'urn:xmpp:omemo:1:bundles:' + registrationId);
    } catch (e) {
      return null;
    }
    let bundle = null;
  ...
  async announceDeviceIds(deviceIds) {
    console.log('[OmemoClient][announceDeviceIds]', deviceIds);

    await this.client.publish(this.client.jid.bare, 'urn:xmpp:omemo:1:devices', {
      deviceList: {
  ...
  async announce(registrationId, identityKeyPair, isNew, removePreKey = null) {
    console.log('[OmemoClient][announce]', {registrationId, isNew});
    const deviceIds = await this.getAnnouncedDeviceIds();

    console.log('[OmemoClient][announce] current deviceIds', deviceIds);
    ...
    if (!deviceIds.has(registrationId)) {
      ...
      // TODO: remove this line once subscription to node is fixed at server side
      await this.storeDeviceIds(this.client.jid.bare, deviceIds);
    ...
    try {
      await this.client.publish(this.client.jid.bare, 'urn:xmpp:omemo:1:bundles:' + registrationId, {
        bundle,
      });
    } catch (e) {
  ...
  async getRecipientSessions(isMUC, recipient) {
    ...
    // let deviceIds = await this.getAnnouncedDeviceIds(recipient);
    let deviceIds = await this.getAnnouncedDeviceIds(recipient, !this.actualizedOpponentDevices.has(recipientBareJid)); //TODO VT temp force request devices from server (before fix on server)

    // if (deviceIds.length === 0 && recipientBareJid !== this.client.jid.bare){
    ...
    console.log('[OmemoClient][getRecipientSessions]', recipient, `, isMUC: ${isMUC}`, ', deviceIds: ', deviceIds);

    if (recipientBareJid === this.client.jid.bare && !deviceIds.has(ownDeviceId)){
      console.log('[OmemoClient][getRecipientSessions] add current device id', deviceIds);
  ...
  async decryptMessage(message) {
    ...
    const localSenderDevices = await this.getAnnouncedDeviceIds(senderJid);

    console.log("[OmemoClient][decryptMessage] currentSenderDevice", currentSenderDevice);
    console.log("[OmemoClient][decryptMessage] localSenderDevices", localSenderDevices);
    if (!localSenderDevices.has(currentSenderDevice)){
      console.warn("[OmemoClient][decryptMessage] force request devices for ", senderJid);
      this.getAnnouncedDeviceIds(senderJid, true);
    }
    ...
    if (keys.length === 0){
      ...
      //   await this.getAnnouncedDeviceIds(message.from.resource, true);
      // }
