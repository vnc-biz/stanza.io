...
export class OmemoClient {
  ...
  async decryptMessage(message) {
    ...
    if (keys.length === 0){
      //TODO VT add force anounce current device
    }
  ...
  async sendMessage(rawMessage, encryptedMsgHint = ENCRYPTED_MSG_DEFAULT_HINT) {
    console.log("[OmemoClient][sendMessage] sendPrivate = ", rawMessage);
    //TODO VT add UTF-8 support
    const omemoMsg = {
      ...rawMessage,
      body: encryptedMsgHint,
      store: true,
      encrypted: await this.createMessage(rawMessage.body, [rawMessage.to, (new JID(rawMessage.from)).bare]),
      encryption: {
  ...
  async sendMessage(rawMessage, members = [rawMessage.to, rawMessage.from], encryptedMsgHint = ENCRYPTED_MSG_DEFAULT_HINT) {
    console.log("[OmemoClient][sendMessage] sendMUC = ", rawMessage, members);
    //TODO VT add UTF-8 support
    const omemoMsg = {
      ...rawMessage,
      body: encryptedMsgHint,
      store: true,
      encrypted: await this.createMessage(rawMessage.body, members),
      encryption: {
        namespace: 'eu.siacs.conversations.axolotl',
        name: 'OMEMO',
      }
