...
export class OmemoClient {
  ...
  async handleDeviceList(msg) {
    console.log('[OmemoClient][handleDeviceList1]', msg);
  ...
  processDevices(devices) {
	...
	console.log("processDevices - devices:", devices);
    // remove duplictes and make sure id is a number
    devices = devices.map(d => {
      d.id = +d.id;
      return d;
    });
	console.log("processDevices - devices mapped:", devices);
	...
	console.log("processDevices - devices processed:", processedDevices);
  ...
  async getAnnouncedDevices(jid, force = true) {
    ...
    if (force || !this.subscriptions.has(jid)) {
      try {
        console.log("[OmemoClient][getAnnouncedDevices] subscribeto devices of user: ", jid);
        await this.client.subscribeToNode(jid, {node: 'urn:xmpp:omemo:1:devices', jid: localUserJid}, (err, data) => {
            console.log("[OmemoClient][getAnnouncedDevices] subscribeto result: ", err, data);
          });
      } catch (e) {
    ...
    try {
      ...
      console.log('[OmemoClient][getAnnouncedDevices] deviceList from server devces-' + jid, jid, deviceList);
    } catch (e) {
  ...
  async announceDevices(devices, newRegistrationId) {
    ...
    const localDeviceId = await this.store.getLocalRegistrationId();
	  ...
	  console.log("[OmemoClient][announcedevices][publishDevices]", this.client.jid.bare, devices);
    await this.client.publishOmemoDevice(this.client.jid.bare, 'urn:xmpp:omemo:1:devices', {
      ...
      console.error("[OmemoClient][announcedevices][publishDevicesResult]", res, err);
    });
  }
  ...
  async announce(device, identityKeyPair, isNew, removePreKey = null, isForceGetAnnouncedDevices = false) {
    ...
    try {
      localStorage.setItem("localOmemoBundle", JSON.stringify(bundle));
    } catch (e) {
      console.log('[OmemoClient][announce] storeBundle error', e);
    }
    try {
  ...
  async decryptMessage(message) {
    ...
    const localDeviceId = await this.store.getLocalRegistrationId();
    // console.log("[OmemoClient][decryptMessage] localDeviceId ", localDeviceId);
    ...
    // console.log("[OmemoClient][decryptMessage] getAnnouncedDevices for ", senderJid);
    let req = this.getAnnouncedDeviceIdsRequests[senderJid];
    if (!req) {
      req = this.getAnnouncedDevices(senderJid, false);
