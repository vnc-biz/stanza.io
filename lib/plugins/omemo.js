...
export class OmemoClient {
  ...
  async storeDeviceIds(jidBare, devices) {
    // convert a set of strings to a set of numbers
    devices = Array.from(devices);
    devices = devices.map(dId => parseInt(dId));
  ...
  async getAnnouncedDeviceIds(jid, force = false) {
    // console.log('[OmemoClient][getAnnouncedDeviceIds]', jid);
    ...
    try {
      ...
      console.log('[OmemoClient][getAnnouncedDeviceIds] deviceList from server', jid, deviceList);
    } catch (e) {
    ...
    try {
      ...
      console.log('[OmemoClient][getAnnouncedDeviceIds] deviceIds', deviceIds);
    } catch (e) {
    ...
    console.log('[OmemoClient][getAnnouncedDeviceIds] return updated list', jid);
  ...
  async announceDeviceIds(deviceIds) {
    console.log('[OmemoClient][announceDeviceIds] deviceIds', deviceIds);
    const localDeviceId = await this.store.getLocalRegistrationId();

    const devicesArray = Array.from(deviceIds);
    const devicesMap = devicesArray.map(dId => {
      return {id: dId || 1, label: "web"};
    });

    console.log('[OmemoClient][announceDeviceIds] devicesMap', devicesMap);
  ...
  async getRecipientSessions(isMUC, recipient) {
    ...
    // let deviceIds = await this.getAnnouncedDeviceIds(recipient, true);
    ...
    console.log('[OmemoClient][getRecipientSessions]', recipient, ', deviceIds: ', deviceIds);
  ...
  async decryptMessage(message) {
    ...
    if (!req) {
      req = this.getAnnouncedDeviceIds(senderJid);
      this.getAnnouncedDeviceIdsRequests[senderJid] = req;
    }
    const localSenderDevices = await req;
    ...
    if (!localSenderDevices.has(currentSenderDevice)){
      ...
      if (!req) {
        console.warn("[OmemoClient][decryptMessage] force request devices for ", senderJid, currentSenderDevice, localSenderDevices);
        req = this.getAnnouncedDeviceIds(senderJid, true);
