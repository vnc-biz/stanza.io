...
export class OmemoClient {
  ...
  async start(platform) {
    ...
    await this.announce(this.buildDeviceInfo(registrationId), identityKeyPair, isNew);
  }
  ...
  async getAnnouncedDevices(jid, force = false) {
    ...
    await this.store.storeDevices(jid, devices);
  ...
  async announceDevices(devices) {
    console.log('[OmemoClient][announceDevices]', devices);
  ...
  async announce(device, identityKeyPair, isNew, removePreKey = null) {
    console.log('[OmemoClient][announce]', {device, isNew});

    let registrationId = device.id;

    const announcedDevices = await this.getAnnouncedDevices();
