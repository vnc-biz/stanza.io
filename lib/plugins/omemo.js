...
export class OmemoClient {
  ...
  async decryptWhisper(message, key) {
    ...
    let whipser;
    if(isMUC) {
      whipser  = await this.store.getWhisper(message.from.toString(), message.id);
    } else {
      whipser  = await this.store.getWhisper(message.from.resource.toString(), message.id);
    }
    ...
    const address = new OmemoAddress(isMUC ? message.from.resource : message.from, message.encrypted.header.sid);
    ...
    await this.store.storeWhisper(isMUC ? message.from.resource : message.from.toString(), message.id, plaintext);
    return plaintext;
  }
